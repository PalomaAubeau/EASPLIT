...
export default function EventScreen({ route, navigation }) {
      ...
      .then((data) => {
        if (data.result) {
          ...
          let remainingBalance =
            data.event && data.event.remainingBalance
              ? data.event.remainingBalance
              : 0;
  ...
  const EventExpense = ({
    ...
    const isOrganizer =
      event.organizer && event.organizer.email === currentUser.email;
    const dispatch = useDispatch();
    ...
    const submitExpense = async () => {
      try {
        if (imageName.trim() === "") {
          ...
          if (data.response) {
            // Utilisez 'response' au lieu de 'success'
            fetchExpenses();
            setExpenseName("");
            setExpenseAmount("");
            setImageName("");
        ...
        <View style={{ marginTop: 30 }}>
          ...
          >
            ...
            <View style={[styles.amount, { marginTop: -10 }]}>
              ...
              <Text style={[styles.textRecap, { marginTop: 0 }]}>
                Solde restant
              </Text>
            </View>
          </View>
