...
export default function CreateEventScreen({ navigation }) {
  ...
  const handleAddParticipant = (participant) => {
    ...
    if (
      !participants.some(
        (p) => p.email.toLowerCase() === participantEmailLowercase
      )
    ) {
      setParticipants([
        ...participants,
        { ...participant, email: participantEmailLowercase },
      ]);
    }
  };
  ...
  const handleEventDateChange = (value) => {
    ...
    if (eventDateObj < today) {
      setErrors((prev) => ({
        ...prev,
        eventDate:
          "La date de l'événement ne peut pas être antérieure à la date du jour.",
      }));
    } else {
  ...
  const handleDeadLineChange = (value) => {
    ...
    if (deadLineObj < today) {
      setErrors((prev) => ({
        ...prev,
        deadLine:
          "La date limite de paiement ne peut pas être antérieure à la date du jour.",
      }));
    } else if (eventDateObj < deadLineObj) {
      setErrors((prev) => ({
        ...prev,
        deadLine:
          "La date limite de paiement doit être antérieure à la date de l'événement.",
      }));
    } else {
  ...
  const handleSubmitEvent = () => {
    if (
      ...
      console.error(
        "Veuillez remplir tous les champs avant de créer l'événement"
      );
      return;
    }
